...
import { BarChart3, Package, ShoppingCart, TrendingUp, Settings, ImageIcon, Target, Users } from "lucide-react"
...
export default function AdminDashboard() {
  ...
  const tabItems = [
    ...
    { value: "users", label: "Usuários", icon: Users, shortLabel: "User" },
      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-8 py-4 lg:py-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4 lg:space-y-6">
          ...
          <div className="hidden lg:block bg-white rounded-xl shadow-sm border border-gray-200 p-2">
            <TabsList className="grid grid-cols-8 gap-2 bg-transparent">
              {tabItems.map((item) => (
                <TabsTrigger
                  key={item.value}
                  value={item.value}
          ...
          <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-200 shadow-2xl">
            <TabsList className="grid grid-cols-4 h-16 bg-transparent rounded-none">
            ...
            <TabsList className="grid grid-cols-4 h-14 bg-gray-50 rounded-none border-t border-gray-100">
