    ...
    <footer className="bg-gradient-to-r from-gray-900 to-gray-800 text-white mt-16">
      <div className="container mx-auto px-4 py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          {/* TODO: add category map to footer */}

          <div>
            <h4 className="text-lg font-semibold mb-4 text-orange-400">Categorias</h4>
            <div className="space-y-2 text-gray-300">
              <Link className="hover:text-orange-400 cursor-pointer transition-colors">
        ...
        {/* {products && products.length > 0 && (
              <div className="space-y-2 text-gray-300">
                {Array.from(new Set(products.map((product) => product.category))).map((category) => (
                  <Button variant="link" key={category} className="hover:text-orange-400 cursor-pointer transition-colors p-0">{category}
                  <p key={category} className="hover:text-orange-400 cursor-pointer transition-colors">{category}</p>
                  </Button>
                ))}


                <p>{products.map((product) => product.category)} produtos</p>
              </div>
            )} */}

        <div className="border-t border-gray-700 mt-8 pt-8 text-center text-gray-400">
          <p>&copy; {new Date().getFullYear()} Casa de Carne Duarte. Todos os direitos reservados.</p>
        </div>
      </div>
    </footer>
  );
